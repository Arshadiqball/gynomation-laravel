...
var KTAppInbox = function() {
    ...
    var record = {
        'parameter' : null,
        'url' : 'user/list/'+startdate.format("YYYY-MM-DD")+'/'+enddate.format("YYYY-MM-DD"),
        'method' : 'POST'
    };
    // var user_list = a_return(record);
    var user_list =null;
            ...
            templates: {
                dropdownItem: function(tagData) {
                    try {
                        ...
                        html += '           <span class="symbol-label" style="background-image: url(\''+ (tagData.pic ? tagData.pic : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT_vaOijrX3OW8v0SJ8J4MsnQkbC8fuycPfYw&usqp=CAU') + '\')">' + (tagData.initials ? tagData.initials : '') + '</span>';
                        html += '       </span>';
    ...
    return {
        ...
        initCompose: function() {
            if($('#kt_inbox_compose_editor').length){
                
                $('#kt_inbox_compose_editor').empty();
...
jQuery(document).ready(function() {
    ...
    $(document).on('click','.email_send', function(){
        ...
        if(email){
            $('#kt_inbox_compose').find('input[name="compose_to"]').val(email);
